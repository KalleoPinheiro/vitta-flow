...
- **Phase / Task**: **Batch A (T1-T7, Phases 1+2) e Batch B (T8-T16, Phases 3+4) completas** — issues #22, #23, #24 e #25 fechadas. Usuário confirmou execução inline (sem sub-agentes) para Batch B; mesma abordagem seguida por padrão para Batch C a menos que o usuário peça delegação. Commits principais: `f9ca946`..`d9fb56e` (Batch A), `832b3a3` (T8), `6a9f3e6` (T9), `bcf8b94` (T10, fecha #24), `f0069da` (T11), `a9b1702` (T12), `7750547` (T13), `deaa2aa` (T14), `23e4294` (T15), `4c44ecc` (T16, fecha #25).
- **Gate completo rodado e verde após cada task de Batch B**: `npm run typecheck`, `npm run lint` (0 erros nos arquivos tocados), `npm run test:coverage --no-file-parallelism` (1907/1907 testes ao final de T16, 97.05% statements — acima do piso de 90%), `npm run check:sv` (OK). WSL seguiu estável (`--no-file-parallelism`, ~5.8GB RAM) em todas as rodadas.
- **Batch C (T17-T24, Phases 5+6, 8 tasks) não iniciado.** Cabe no orçamento inline (~8 tasks) — não exige oferta de sub-agente pela regra da skill.
- **Nenhum Verifier de feature rodou ainda** — só roda automaticamente após a ÚLTIMA task do épico (T24) ou de um grupo de prioridade entregue isoladamente.
- **Padrão consolidado ao longo de Batch B** (útil para Batch C): cada entidade isolada segue o mesmo molde — (1) repositório Drizzle ganha `clinicId: string | null` no construtor, usa `withTenant` em toda leitura, e `save()`/`delete()` lança erro se `clinicId` for `null`; (2) `container.ts` passa `tenant.clinicId`; (3) call sites de teste que instanciam o repositório direto (`new Drizzle...Repository(appDb)`) precisam de um clinicId literal (`"legacy-clinic"`); (4) rotas de leitura (GET) usam `guard.session?.clinicId ?? null` (papel de sistema pode ler cross-empresa); rotas de escrita usam `?? LEGACY_CLINIC_ID` (nunca `null`, já que o repositório rejeitaria); (5) todo `getRepositories({ clinicId: null })` num arquivo tocado precisa ser auditado — call sites fora do escopo da task atual que **escrevem** via um repositório recém-scoped quebram silenciosamente (aconteceu 2x: rotas do portal de agendamento em T8, teste de idade de foto pendente em T16) — sempre rodar a suíte COMPLETA (não só `tests/api tests/infrastructure`) pelo menos uma vez por task para pegar isso. (6) Nova task de isolamento sempre ganha `tests/api/<entidade>-tenant-isolation.test.ts` usando `tests/support/clinics.ts` (`ensureTestClinics`, `CLINIC_A_ID`, `CLINIC_B_ID`).
- **Decisão registrada nesta sessão, ainda não como AD formal**: `PatientRepository.findClinicIdById` (T7) — método auxiliar mínimo para o acesso cross-empresa do papel de sistema auditar com o `clinicId` real do recurso, sem poluir o domínio `Patient`. Mesma necessidade resolvida de outra forma em T16: `DrizzleConditionPhotoRepository` ganhou `clinicId` no construtor E `withTenant` em toda query (não só o storage em disco) — sem isso, uma sessão cross-empresa conseguiria sobrescrever/apagar METADADOS de foto de outra clínica mesmo com o binário protegido pelo path do storage.
- **Next step**: iniciar T17 (Isolamento de Suprimento, Lote de Suprimento e Movimento de Estoque — `drizzle-inventory-repositories.ts`), primeira e única task da Phase 5 (M5/#26), Gate: full.
- **Blockers**: nenhum.

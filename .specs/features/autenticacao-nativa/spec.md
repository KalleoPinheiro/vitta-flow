...
| Falha no envio do e-mail de convite durante o cadastro | A conta permanece criada e a rota responde 201 normalmente; a falha é logada. O convite pode ser reemitido pelo fluxo de reset de senha | A alternativa (desfazer o cadastro) transformaria uma indisponibilidade do provedor de e-mail numa falha de cadastro, e o fluxo de reset já cobre a reemissão | y |
...
- IF um token é apresentado com `purpose` diferente do esperado pela rota THEN o sistema SHALL tratá-lo como inválido.

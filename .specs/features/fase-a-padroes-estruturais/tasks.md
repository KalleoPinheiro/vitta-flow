...
**Status**: Concluído — todas as 34 tasks (T1-T34) entregues. Phase 1 done (T1-T5, commits ed31d8f, a912f98, 821d2a7, e174ecd; T2 sem commit — ErrorAlert já único). Desvio documentado: T3/T4 estenderam escopo a `pacientes/[id]/page.tsx` (propagar isLoading/error do pai). Phase 2 done (T6-T17, commits 8d1e762..8bd4ae6). Desvio documentado em T17: o e2e revelou que envolver as tabelas em `overflow-x-auto` não bastava para AC4 da história P1 — `SidebarProvider` (@still-void/ui) já embrulha os filhos num `.sv-app-shell` flex, e o `<div className="flex min-h-screen">` do próprio shell (`staff-layout-client.tsx`) herdava `min-width: auto` como item flex único dessa linha, crescendo para caber o `<main>` e empurrando a página inteira além do viewport mobile. Fix (`min-w-0` no wrapper + `min-h-11/min-w-11` no `SidebarTrigger` para a área de toque de 44px) entrou no commit de T17 por ser pré-requisito do próprio teste, não constatado no Design original. 2484/2484 testes unit, 82/82 e2e (1 flaky por colisão de horário, absorvida pelo retry padrão da suíte).
...
Phase 4 done (T31-T34, commits f48df32, 5b5e095, df10ef8, c026a44). Cada uma das 4 páginas-alvo (`pacientes/page.tsx`, `configuracoes/page.tsx`, `portal/patient-view.tsx`, `portal/consent-card.tsx`) passou a chamar `useToast` (`@still-void/ui/react/client`) nos handlers de mutação já existentes, replicando exatamente a forma de chamada já usada em `faturamento/page.tsx` e `(staff)/page.tsx` (`toast({ description, variant: "success" | "danger" })`). Em todos os 4 casos o alerta inline existente (`ErrorAlert`/`setActionError`/`setError`) foi preservado — o toast soma-se a ele, não o substitui —, então os testes de erro que antes buscavam um único texto agora usam `getAllByText(...).length >= 2` (ErrorAlert inline + toast) quando a mensagem é idêntica nos dois lugares. Escopo mantido estritamente ao texto de cada task: em `pacientes/page.tsx`, criar/editar (`handleSubmit`) e ativar/desativar (`toggleActive`); em `configuracoes/page.tsx`, só `AccountsSection` — criar conta (`AccountForm.handleSubmit` via `onSaved`) e ativar/desativar conta (`toggleActive`), ficando de fora `ScheduleSection.save` (grade de horários) e `resendInvite` por não estarem no texto "criar/editar/desativar conta" da task; em `portal/patient-view.tsx`, só `confirmAppointment`; em `portal/consent-card.tsx`, só `ConsentCard.accept` (não `PatientPhotoUpload`, fora do texto da task "ação de assinar/consentir"). Como `useToast()` lança se chamado fora de um `ToastProvider`, todo `render(...)` dessas 4 páginas/componentes nos testes existentes foi trocado para `renderWithToast` (`tests/support/render-with-toast.tsx`, já usado desde a Fase 1) — sem isso os testes já existentes quebrariam mesmo sem tocar a asserção. Nenhum teste foi enfraquecido ou removido; todas as extensões são aditivas. 2500/2500 testes unit (contagem de `it` não mudou — extensões de asserção em testes existentes, sem novos casos, coerente com o "Tests" de cada task). Gate de build final da feature completa: `typecheck` ok, `lint` ok (`ESLint: No issues found`), `check:sv` ok (adoção do @still-void/ui v2 completa), `npm test` 163 arquivos / 2500 testes passando, `test:coverage` 96.65% statements / 91.17% branches / 96.56% functions / 96.72% lines (acima do mínimo de 90% do AGENTS.md).

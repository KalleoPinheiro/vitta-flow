      ...
      <Table className="w-full text-left text-sm">
        ...
        <TableBody>
          {patients.map((patient) => (
            <TableRow
              key={patient.id}
              className={patient.active ? '' : 'bg-surface-2/60'}
            >
              ...
              <TableCell className="px-4 py-3 text-ink-2">
                ...
                <div className="text-ink-3 text-xs">{patient.phone}</div>
              </TableCell>
              <TableCell className="px-4 py-3 text-ink-2">
...
export default function PatientsPage() {
  ...
  const handleSubmit = async (values: PatientFormValues) => {
    ...
    try {
      ...
      toast({
        description:
          editing === 'new' ? 'Paciente criado' : 'Paciente atualizado',
        variant: 'success',
      });
    } catch (err) {
      toast({
  ...
  return (
    <div>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <h1 className="sv-display font-bold text-2xl">Pacientes</h1>
        <Button
          type="button"
      ...
      <LoadMoreButton
        visible={Boolean(patients) && hasMore}
        onClick={loadMore}
      />

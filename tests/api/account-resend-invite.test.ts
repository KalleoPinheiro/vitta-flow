...
describe("Feature: POST /api/accounts/[id]/resend-invite", () => {
  ...
  const createAccount = async (clinicId: string, email: string) => {
    ...
    const response = await route.POST(
      jsonRequest("/api/accounts", "POST", { email, role: "atendente" }, adminCookieHeader(clinicId)),
    );
    emails.restore();
  ...
  const resend = async (id: string, clinicId: string) => {
    ...
    const response = await route.POST(
      jsonRequest(`/api/accounts/${id}/resend-invite`, "POST", undefined, adminCookieHeader(clinicId)),
  ...
  it("Dada conta desativada, Quando reenviar, Então 400", async () => {
    ...
    await byIdRoute.PATCH(
      jsonRequest(`/api/accounts/${id}`, "PATCH", { active: false }, adminCookieHeader(CLINIC_A_ID)),
      { params: Promise.resolve({ id }) },
    );
  ...
  it("Dada nenhuma sessão, Quando reenviar, Então 401", async () => {
    ...
    const response = await route.POST(
      new NextRequest(`http://localhost/api/accounts/${id}/resend-invite`, { method: "POST" }),
      { params: Promise.resolve({ id }) },
    );

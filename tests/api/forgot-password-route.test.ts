...
const createAccount = async (email: string): Promise<void> => {
  ...
  try {
    const headers = cookieHeaderFor("company_admin", "admin-fp@example.com", CLINIC_A_ID);
    await route.POST(jsonRequest("/api/accounts", "POST", { email, role: "atendente" }, headers));
  } finally {
    emails.restore();
  }
};

const forgot = async (email: string, headers: Record<string, string> = freshIp()) => {
  const route = await import("@/app/api/auth/forgot-password/route");
  return route.POST(jsonRequest("/api/auth/forgot-password", "POST", { email }, headers));
};
...
describe("Feature: POST /api/auth/forgot-password", () => {
  ...
  it("Dado um corpo sem e-mail, Quando pedir reset, Então ainda responde 200 com a mensagem neutra", async () => {
    ...
    const response = await route.POST(
      jsonRequest("/api/auth/forgot-password", "POST", { naoEhEmail: 1 }, freshIp()),
    );

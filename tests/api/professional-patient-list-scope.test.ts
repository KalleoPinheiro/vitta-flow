...
describe("Feature: Escopo do Profissional em listagens (RBAC-17/19)", () => {
  ...
  const createPatientAs = async (fullName: string, email: string, professionalId?: string) => {
    const route = await import("@/app/api/patients/route");
    const headers = professionalId
      ? cookieHeaderFor("profissional", `${professionalId}@x.com`, CLINIC_A_ID, professionalId)
      : adminCookieHeader(CLINIC_A_ID);
    const response = await route.POST(
      jsonRequest("/api/patients", "POST", { fullName, email, phone: "11999990000" }, headers),
    );
    const body = (await response.json()) as Envelope<{ id: string }>;
    return body.data.id;
  };
  ...
  describe("Cenário: GET /api/patients", () => {
    it("Dado profissional com um paciente vinculado e outro alheio na clínica, Quando listar, Então só vê o próprio", async () => {
      ...
      const meuPaciente = await createPatientAs("Meu Paciente", "meu-paciente@x.com", drVinculo);
      const outroPaciente = await createPatientAs("Paciente Alheio", "paciente-alheio@x.com");
      ...
      const response = await route.GET(jsonRequest("/api/patients", "GET", undefined, headers));
      const body = (await response.json()) as Envelope<{ id: string }[]>;
  ...
  describe("Cenário: GET /api/appointments", () => {
    ...
    it("Dado dois profissionais com agendamentos distintos, Quando um lista, Então só vê a própria agenda mesmo pedindo professionalId de outro", async () => {
      ...
      const patientA = await createPatientAs("Paciente Dr. A", "paciente-dr-a@x.com");
      const patientB = await createPatientAs("Paciente Dr. B", "paciente-dr-b@x.com");

      await scheduleAs(patientA, drA, admin);
      await scheduleAs(patientB, drB, admin);

      const route = await import("@/app/api/appointments/route");
      const headersA = cookieHeaderFor("profissional", `${drA}@x.com`, CLINIC_A_ID, drA);
      ...
      const body = (await response.json()) as Envelope<{ professionalId: string }[]>;

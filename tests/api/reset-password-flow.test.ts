...
const accountWithPassword = async (email: string, password: string): Promise<void> => {
  ...
  try {
    const headers = cookieHeaderFor("company_admin", "admin-rf@example.com", CLINIC_A_ID);
    await accounts.POST(
      jsonRequest("/api/accounts", "POST", { email, role: "atendente" }, headers),
    );
    const invite = tokenFromLastEmail(emails);
    const response = await setPassword.POST(
      jsonRequest("/api/auth/set-password", "POST", { token: invite, password }, freshIp()),
    );
...
const setPasswordWith = async (token: string, password: string) => {
  ...
  return route.POST(
    jsonRequest("/api/auth/set-password", "POST", { token, password }, freshIp()),
  );
};
...
describe("Feature: Ciclo completo de reset de senha", () => {
  ...
  it("Dado um reset concluído, Quando logar com a senha nova, Então 200; com a antiga, 401", async () => {
    ...
    expect((await login("ciclo-reset@x.com", "senha-antiga-1")).status).toBe(401);
  });
  ...
  it("Dado um segundo pedido de reset, Quando usar o primeiro link, Então 400 e o segundo continua válido", async () => {
    ...
    expect((await login("reset-reemitido@x.com", "senha-nova-5b")).status).toBe(200);
  });
});

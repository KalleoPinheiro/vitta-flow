...
describe('Feature: Plano de cuidados (SAE) — casos de uso', () => {
  ...
  describe('Cenário: abrir plano de cuidados', () => {
    it('Dado paciente existente, Quando abrir plano, Então plano ativo persistido', async () => {
      const plan = await new OpenCarePlan(
        carePlanRepo,
        patientRepo,
        conditionRepo,
      ).execute({
  ...
  describe('Cenário: registrar execução de intervenção', () => {
    ...
    it('Dado intervenção inexistente, Quando registrar execução, Então lança NotFoundError', async () => {
      await expect(
        new RecordIntervention(
          recordRepo,
          interventionRepo,
          carePlanRepo,
        ).execute({ interventionId: 'ghost' }),
      ).rejects.toThrow(NotFoundError);
    });
  ...
  describe('Cenário: resolver e listar planos', () => {
    it('Dado plano ativo, Quando resolver, Então status muda para resolved', async () => {
      ...
      const resolved = await new ResolveCarePlan(carePlanRepo).execute({
        id: plan.id,
      });
    ...
    it('Dado plano inexistente, Quando resolver, Então lança NotFoundError', async () => {
      await expect(
        new ResolveCarePlan(carePlanRepo).execute({ id: 'ghost' }),
      ).rejects.toThrow(NotFoundError);
    });

    it('Dado múltiplos planos do paciente, Quando listar, Então retorna todos', async () => {
      await new OpenCarePlan(carePlanRepo, patientRepo, conditionRepo).execute({
        patientId: maria.id,
      });
      await new OpenCarePlan(carePlanRepo, patientRepo, conditionRepo).execute({
        patientId: maria.id,
      ...
      const plans = await new ListCarePlansByPatient(carePlanRepo).execute({
        patientId: maria.id,
      });
  ...
  describe('Cenário: montar visão completa do plano (GetCarePlan)', () => {
    it('Dado plano com diagnóstico, resultado avaliado e intervenção executada, Quando buscar, Então agrega tudo', async () => {
      ...
      await new AddCarePlanDiagnosis(
        diagnosisRepo,
        carePlanRepo,
        diagnosisCatalog,
      ).execute({
      ...
      await new EvaluateOutcome(
        evaluationRepo,
        outcomeRepo,
        carePlanRepo,
      ).execute({ outcomeId: outcome.id, score: 3 });
      await new RecordIntervention(
        recordRepo,
        interventionRepo,
        carePlanRepo,
      ).execute({
      ...
      expect(
        detail.outcomes[0].outcome.currentScore(detail.outcomes[0].evaluations),
      ).toBe(3);

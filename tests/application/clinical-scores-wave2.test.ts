...
describe("Feature O2.2: Escala DET e complicações canônicas", () => {
  ...
  it("Dado código de complicação inválido, Quando criar, Então ValidationError", () => {
    expect(() =>
      ConditionAssessment.create({ ...baseAssessment, complicationCodes: "dermatite-x" }),
    ).toThrow(ValidationError);
  });
});
...
describe("Feature O2.3: Recall de 1 clique (follow-up scheduled)", () => {
  ...
  it("Dado follow-up scheduled, Quando concluir, Então done; re-agendar é erro", () => {
    ...
    expect(() => scheduled.markScheduled()).toThrow(InvalidStatusTransitionError);
  });
});
...
describe("Feature O2.4: Kit de insumos por procedimento", () => {
  ...
  beforeEach(async () => {
    ...
    procedure = Procedure.create({ name: "Troca de bolsa", priceCents: 25000, durationMinutes: 60 });

    const create = new CreateSupply(supplyRepo);
    const register = new RegisterStockMovement(supplyRepo, movementRepo);
    const bolsa = await create.execute({ name: "Bolsa", unit: "un", minQty: 2, priceCents: 4000 });
    const placa = await create.execute({ name: "Placa", unit: "un", minQty: 2, priceCents: 6000 });
    bolsaId = bolsa.id;
    placaId = placa.id;
    await register.execute({ supplyId: bolsaId, type: "in", quantity: 10, reason: "Compra" });
    await register.execute({ supplyId: placaId, type: "in", quantity: 1, reason: "Compra" });

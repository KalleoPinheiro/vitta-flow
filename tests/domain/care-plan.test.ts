...
describe('Feature: Plano de cuidados (SAE)', () => {
  describe('Cenário: abrir plano de cuidados', () => {
    ...
    it('Dado patientId vazio, Quando criar, Então lança ValidationError', () => {
      expect(() => CarePlan.create({ patientId: '  ' })).toThrow(
        ValidationError,
      );
    });
  });
...
describe('Feature: Avaliação de resultado (OutcomeEvaluation) — somente inserção, nunca sobrescreve', () => {
  ...
  it('Dado pontuação fora de 1–5, Quando criar, Então lança ValidationError', () => {
    expect(() =>
      OutcomeEvaluation.create({ outcomeId: 'o1', score: 0 }),
    ).toThrow(ValidationError);
    expect(() =>
      OutcomeEvaluation.create({ outcomeId: 'o1', score: 6 }),
    ).toThrow(ValidationError);
  });

  it('Dado outcomeId vazio, Quando criar, Então lança ValidationError', () => {
    expect(() =>
      OutcomeEvaluation.create({ outcomeId: ' ', score: 3 }),
    ).toThrow(ValidationError);
  });
...
describe('Feature: Execução de intervenção (InterventionRecord) — somente inserção, nunca sobrescreve', () => {
  it('Dado interventionId válido, Quando criar, Então registro criado com performedAt', () => {
    const record = InterventionRecord.create({
      interventionId: 'i1',
      professionalId: 'prof1',
    });
  ...
  it('Dado interventionId vazio, Quando criar, Então lança ValidationError', () => {
    expect(() => InterventionRecord.create({ interventionId: '  ' })).toThrow(
      ValidationError,
    );
  });

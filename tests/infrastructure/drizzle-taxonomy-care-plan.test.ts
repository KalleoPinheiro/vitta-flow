...
describe("Feature: Persistência PostgreSQL — taxonomias de enfermagem e plano de cuidados", () => {
  ...
  it("Dado diagnóstico NANDA-I salvo, Quando buscar por código/lote/termo, Então roundtrip correto", async () => {
    ...
    expect((await repo.findByCode("00046"))?.label).toBe("Integridade da pele prejudicada");
  ...
  it("Dado ligações NANDA→NOC/NIC salvas, Quando buscar por diagnóstico e papel, Então filtra corretamente", async () => {
    ...
    await repo.save(TaxonomyLinkage.create({ diagnosisCode: "00046", role: "outcome", targetCode: "1101" }));
    await repo.save(
  ...
  it("Dado plano de cuidados completo, Quando salvar e consultar, Então roundtrip de diagnóstico/resultado/intervenção", async () => {
    ...
    expect((await carePlanRepo.findById(plan.id))?.professionalId).toBe(professional.id);
    ...
    expect(storedDiagnoses[0].definingCharacteristics).toBe("Ruptura da epiderme");
    ...
    expect((await interventionRepo.findById(intervention.id))?.frequency).toBe("A cada troca de placa");

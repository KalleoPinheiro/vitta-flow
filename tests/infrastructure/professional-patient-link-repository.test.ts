...
describe("Feature: Repositório de vínculo Profissional-Paciente (R4, RBAC-17/21)", () => {
  ...
  beforeAll(async () => {
    ...
    await db.execute(
      sql.raw(`INSERT INTO "clinics" (id, name, created_at, created_by) VALUES ('${CLINIC_ID}','Clinica Links',now(),'system')`),
    );
    await db.execute(
      sql.raw(`INSERT INTO "clinics" (id, name, created_at, created_by) VALUES ('${CLINIC_OTHER_ID}','Outra Clinica',now(),'system')`),
    );
    await db.execute(
      sql.raw(`INSERT INTO "professionals" (id, clinic_id, full_name, active, created_at) VALUES ('prof-1','${CLINIC_ID}','Dr. A',true, now())`),
    );
    await db.execute(
      sql.raw(`INSERT INTO "patients" (id, clinic_id, full_name, email, phone, active, created_at) VALUES ('pat-1','${CLINIC_ID}','Paciente 1','pat1@x.com','111',true, now())`),
    );
    await db.execute(
      sql.raw(`INSERT INTO "patients" (id, clinic_id, full_name, email, phone, active, created_at) VALUES ('pat-2','${CLINIC_ID}','Paciente 2','pat2@x.com','222',true, now())`),
    );
  });
  ...
  describe("Cenário: ensureLink é idempotente", () => {
    it("Dado o mesmo par profissional-paciente, Quando ensureLink 2x, Então não duplica nem lança", async () => {
      ...
      const rows = await db.execute(
        sql.raw(`SELECT count(*)::int AS c FROM "professional_patient_links" WHERE professional_id = 'prof-1' AND patient_id = 'pat-1'`),
      );
  ...
  describe("Cenário: findLinkedPatientIds escopado por profissional e clínica", () => {
    ...
    it("Dado repositório de outra clínica, Quando findLinkedPatientIds do mesmo professionalId, Então lista vazia (isolamento por empresa)", async () => {
      const otherClinicRepo = new DrizzleProfessionalPatientLinkRepository(appDb, CLINIC_OTHER_ID);
  ...
  describe("Cenário: papel de sistema não cria vínculo", () => {
    it("Dado repositório com clinicId nulo, Quando ensureLink, Então lança erro", async () => {
      const systemRepo = new DrizzleProfessionalPatientLinkRepository(appDb, null);

...
describe("Feature: Migração de backfill de role em user_accounts (RBAC-01)", () => {
  ...
  beforeAll(async () => {
    ...
    const priorMigrationsDir = fs.mkdtempSync(path.join(process.cwd(), ".tmp-migrate-role-"));
  ...
  it("preenche role='company_admin' em 100% das contas existentes, zero linha órfã", async () => {
    const total = await db.execute(sql.raw(`SELECT count(*)::int AS c FROM "user_accounts"`));
    const orphans = await db.execute(
      sql.raw(`SELECT count(*)::int AS c FROM "user_accounts" WHERE role IS NULL OR role <> 'company_admin'`),
    );

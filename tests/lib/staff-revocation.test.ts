...
describe("Feature: Revogação de sessão de conta staff desativada (SEC1-01..04)", () => {
  ...
  it("Dado conta existente e inativa, Quando checar, Então sessão revogada", async () => {
    ...
    expect(await isStaffSessionRevoked(adminSession("ana@clinica.com"), lookup)).toBe(true);
  });
  ...
  it("Dado conta existente e ativa, Quando checar, Então sessão válida", async () => {
    ...
    expect(await isStaffSessionRevoked(adminSession("ana@clinica.com"), lookup)).toBe(false);
  });

  it("Dado subject 'local' (senha master), Quando checar, Então válida sem consultar o banco", async () => {
    const lookup = vi.fn();

    expect(await isStaffSessionRevoked(adminSession("local"), lookup)).toBe(false);
  ...
  it("Dado email sem conta (login Google via allowlist), Quando checar, Então válida (deny-list, AD-001)", async () => {
    ...
    expect(await isStaffSessionRevoked(adminSession("google@clinica.com"), lookup)).toBe(false);
  });

  it("Dado papel patient/partner, Quando checar, Então válida sem consultar o banco", async () => {
    const lookup = vi.fn();
    const patient: Session = { expiresAtMs: Date.now() + 60_000, subject: "p@x.com", role: "patient", clinicId: "legacy-clinic", professionalId: null };
    const partner: Session = { expiresAtMs: Date.now() + 60_000, subject: "d@x.com", role: "partner", clinicId: "legacy-clinic", professionalId: null };
  ...
  it("Dado checagem repetida dentro de 60s, Quando checar de novo, Então usa cache (uma consulta só)", async () => {
    ...
    const revoked = await isStaffSessionRevoked(adminSession("ana@clinica.com"), lookup, now + 30_000);
